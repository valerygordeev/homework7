...
1. Устанавливаем ПО:
     ...
     yum install -y gcc
1. Выбираем ПО веб-сервер nginx и openssl
1. Скачиваем последний пакет SRPM nginx 
     wget https://nginx.org/packages/centos/8/SRPMS/nginx-1.24.0-1.el8.ngx.src.rpm
1. Создаем директорию /root/rpmbuild
     mkdir rpmbuild
1. Устанавливаем пакет из скачанного файла 
     rpm -i nginx-1.*
1. Скачиваем openssl
     wget https://github.com/openssl/openssl/archive/refs/heads/OpenSSL_1_1_1-stable.zip
1. Разархивируем архив /root/rpmbuild/SOURCES/nginx-1.24.0.tar.gz
     tar -xvf nginx-1.24.0.tar.gz
1. Загружаем зависимости 
     yum-builddep rpmbuild/SPECS/nginx.spec
1. Добавляем в ~/rpmbuild/SPECS/nginx.spec в раздел %build путь к openssl
     %build
...
1. Проверяем результат визуально
     ll rpmbuild/RPMS/x86_64/
1. Устанавиваем собранный веб-сервер 
     yum localinstall -y rpmbuild/RPMS/x86_64/nginx-1.24.0-1.el8.ngx.x86_64.rpm
1. Запускаем и проверяем статус веб-сервера
     systemctl start nginx
     systemctl status nginx
1. Создаем локальный репозиторий. Создаем директорию repo в директории веб-сервера /usr/share/nginx/html
     mkdir /usr/share/nginx/html/repo
1. Копируем собранный rpm и загружаем rpm Percona-Server'а для установки репозитория
...
1. Включаем просмотр каталогов в nginx. Добавляем в раздел location файла /etc/nginx/conf.d/default.conf добавляем директиву "autoindex on"
     server {
     listen       80;
...
1. Проверяем синтаксис nginx и перезагружаем nginx
     nginx -t
     nginx -s reload
1. Смотрим веб-страницу репозитория
...
1. Добавляем в конфигурационный файл репозитария yum подключенние локальный репозиторий
     ...
     enabled=1
     EOF
1. Проверяем наличие в репозитории yum наличие программ локального репозитория
     yum repolist enabled | grep otus
     yum list | grep otus
1. Проверяем работу репозтория установкой 
